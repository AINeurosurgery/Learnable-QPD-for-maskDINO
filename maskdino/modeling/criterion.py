            ...
            for i, aux_outputs in enumerate(outputs["aux_outputs"]):
                ...
                if i>=start:
                    if self.dn != "no" and mask_dict is not None:
                        ...
                        losses.update(l_dict)
        #####################################################
        losses.update({'loss_ce_0': torch.as_tensor(0.).to('cuda')})
        losses.update({'loss_ce_1': torch.as_tensor(0.).to('cuda')})
        # losses.update({'loss_ce_2': torch.as_tensor(0.).to('cuda')})
        # losses.update({'loss_ce_3': torch.as_tensor(0.).to('cuda')})
        # losses.update({'loss_ce_4': torch.as_tensor(0.).to('cuda')})
        # factor = 0.5
        # names = ['loss_ce_0','loss_ce_1','loss_ce_dn_0','loss_ce_dn_1']
        # # names = []
        # # names.append('loss_bbox_0')
        # # names.append('loss_bbox_1')
        # # names.append('loss_bbox_dn_0')
        # # names.append('loss_bbox_dn_1')
        # # names.append('loss_giou_0')
        # # names.append('loss_giou_1')
        # # names.append('loss_giou_dn_0')
        # # names.append('loss_giou_dn_1')
        # # names.append('loss_mask_0')
        # # names.append('loss_mask_1')
        # # names.append('loss_mask_dn_0')
        # # names.append('loss_mask_dn_1')
        # # names.append('loss_dice_0')
        # # names.append('loss_dice_1')
        # # names.append('loss_dice_dn_0')
        # # names.append('loss_dice_dn_1')

        # for now_name in names:
        #     losses[now_name] = losses[now_name]*factor
