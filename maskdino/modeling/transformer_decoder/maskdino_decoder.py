...
class MaskDINODecoder(nn.Module):
    ...
    def forward(self, x, mask_features, masks, targets=None):
        ...
        if self.two_stage:
            ...
            #Rohan anna's version
            # all_confidence1 = torch.topk(enc_outputs_class_unselected.max(-1)[0], enc_outputs_class_unselected.shape[1], dim=1)[0]
            # to_nms_box = box_ops.box_cxcywh_to_xyxy(enc_outputs_coord_unselected.sigmoid())
            # final_output_indices = []
            # for ind11 in range(to_nms_box.shape[0]):
            #     all_confidence2 = (all_confidence1[ind11]-all_confidence1[ind11,-1])/(all_confidence1[ind11,0]-all_confidence1[ind11,-1])
            #     out_from_nms_index = torchvision.ops.nms(to_nms_box[ind11],all_confidence2,0.5)
            #     # print(out_from_nms_index.shape)
            #     if len(out_from_nms_index)<300:
            #         out_from_nms_index = torchvision.ops.nms(to_nms_box[ind11],all_confidence2,0.6)
            #     if len(out_from_nms_index)<300:
            #         out_from_nms_index = torchvision.ops.nms(to_nms_box[ind11],all_confidence2,0.7)
            #     if len(out_from_nms_index)<300:
            #         out_from_nms_index = torchvision.ops.nms(to_nms_box[ind11],all_confidence2,0.8)
            #     if len(out_from_nms_index)<300:
            #         out_from_nms_index = torchvision.ops.nms(to_nms_box[ind11],all_confidence2,0.9)
            #     if len(out_from_nms_index)<300:
            #         out_from_nms_index = torchvision.ops.nms(to_nms_box[ind11],all_confidence2,1.0)
            #     final_output_indices.append(out_from_nms_index[:topk])
            
            all_confidence1 = enc_outputs_class_unselected.max(-1)[0]
            to_nms_box = box_ops.box_cxcywh_to_xyxy(enc_outputs_coord_unselected.sigmoid())
            final_output_indices = []
            for ind11 in range(to_nms_box.shape[0]):
                all_confidence2 = (all_confidence1[ind11]-torch.min(all_confidence1[ind11]))/(torch.max(all_confidence1[ind11])-torch.min(all_confidence1[ind11]))
                out_from_nms_index = torchvision.ops.nms(to_nms_box[ind11],all_confidence2,0.5)
                # print(out_from_nms_index.shape)
                if len(out_from_nms_index)<300:
                    out_from_nms_index = torchvision.ops.nms(to_nms_box[ind11],all_confidence2,0.6)
                ...
                # output_nms_indices.append(out_from_nms_index)
                #out_from_nms_boxes = to_nms_box[out_from_nms_index]
                #final_output_boxes.append[out_from_nms_boxes[:topk]]
            ...
            #all_bbox = torch.gather(enc_outputs_coord_unselected, 1, all_proposals.unsqueeze(-1).repeat(1, 1, 4))
            #all_confidence = torch.gather(enc_outputs_class_unselected, 1, all_proposals.unsqueeze(-1).repeat(1, 1, 7))
            
            #torch.save(all_bbox.detach(),"/home/rohan_rd/scratch/maskdino/MaskDINO/demo/save_rpn_outputs_intm/bbox.pt")
        ...
        #torch.save(predictions_mask,"/home/arnav/scratch/maskdino/MaskDINO/intermediate_results/pred_mask.pt")
        # torch.save(predictions_class,"/home/rohan_rd/scratch/maskdino/MaskDINO/demo/save_rpn_outputs_intm/predictions_class.pt")
        # torch.save(out_boxes,"/home/rohan_rd/scratch/maskdino/MaskDINO/demo/save_rpn_outputs_intm/out_box.pt")
