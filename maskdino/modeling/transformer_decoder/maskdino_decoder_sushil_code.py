...
class MaskDINODecoder(nn.Module):
    ...
    def forward(self, x, mask_features, masks, targets=None):
        ...
        if self.two_stage:
            ...
            #############################################################
            temp = {"index":[], "instrument_class":[], "instrument_confidence":[]}
            for idx, item in enumerate(enc_outputs_class_unselected[0]):
                temp["index"].append(idx)
                temp["instrument_class"].append(item.max(-1)[1].cpu().item())
                temp["instrument_confidence"].append(item.max(-1)[0].cpu().item())
            temp = pd.DataFrame(temp)
            with open("/home/rohan_rd/scratch/maskdino/MaskDINO/demo/check_val_gt_anns.txt", 'r') as f:
                text = f.read()
            if text != "":
                gt = list(map(int, text.split(",")))
                num_bbox_per_gt_class = int(self.num_queries/len(gt))
                indexes = []    
                for class_num in gt:
                    class_df = temp[temp["instrument_class"] == class_num]
                    class_df = class_df.sort_values(by=["instrument_confidence"], ascending=False)
                    indexes += class_df["index"].iloc[:num_bbox_per_gt_class].tolist()
                new_topk_proposals = torch.tensor(indexes).unsqueeze(0)
                topk_proposals = new_topk_proposals.cuda()

                # top300_bbox_new = torch.gather(enc_outputs_coord_unselected, 1, topk_proposals.unsqueeze(-1).repeat(1, 1, 4))
                # top300_confidence_new = torch.gather(enc_outputs_class_unselected, 1, topk_proposals.unsqueeze(-1).repeat(1, 1, 7))
                # torch.save((top300_bbox_new.detach(), top300_confidence_new.detach()), "/home/rohan_rd/scratch/maskdino/MaskDINO/output/proposals_new.pth")

                # top300_bbox_old = torch.gather(enc_outputs_coord_unselected, 1, torch.topk(enc_outputs_class_unselected.max(-1)[0], self.num_queries, dim=1)[1].unsqueeze(-1).repeat(1, 1, 4))
                # top300_confidence_old = torch.gather(enc_outputs_class_unselected, 1, torch.topk(enc_outputs_class_unselected.max(-1)[0], self.num_queries, dim=1)[1].unsqueeze(-1).repeat(1, 1, 7))
                # torch.save((top300_bbox_old.detach(), top300_confidence_old.detach()), "/home/rohan_rd/scratch/maskdino/MaskDINO/output/proposals_old.pth")

            else:
                topk_proposals = torch.topk(enc_outputs_class_unselected.max(-1)[0], self.num_queries, dim=1)[1]
            # all_proposals = torch.topk(enc_outputs_class_unselected.max(-1)[0], enc_outputs_class_unselected.shape[1], dim=1)[1]
            
            # all_bbox = torch.gather(enc_outputs_coord_unselected, 1, all_proposals.unsqueeze(-1).repeat(1,s.unsqueeze(-1).repeat(1, 1, 7))
            # all_confidence = torch.gather(enc_outputs_class_unselected, 1, all_proposals.unsqueeze(-1).repeat(1, 1, 7))
            # torch.save(all_bbox.detach(),"/home/rohan_rd/scratch/maskdino/MaskDINO/ 1, 4))
            # all_confidence = torch.gather(enc_outputs_class_unselected, 1, all_proposaldemo/save_rpn_outputs_intm/bbox.pt")
            # torch.save(all_confidence.detach(),"/home/rohan_rd/scratch/maskdino/MaskDINO/demo/save_rpn_outputs_intm/confidence.pt")
            ##############################################################
            ...
            ##############################################################
            # topk_proposals = torch.topk(enc_outputs_class_unselected.softmax(-1).max(-1)[0], topk, dim=1)[1]
            # our_top = []
            # temp = enc_outputs_class_unselected.argmax(-1)[0].tolist()
            # for item in topk_proposals:
            #     cls = temp[item]
            #     if cls==2:
            #         our_top.append(item)
            # topk_proposals = torch.tensor(our_top[:300]).unsqueeze(0).cuda()
            #################################################################
